// ============================================================================
// 🤖 TOOL SUPABASE POUR CHARLIE & LÉO - VERSION N8N V4 FINALE
// ============================================================================
// ✅ CODE COMPLET avec TOUTES les fonctionnalités + recherche corrigée
...
async function supabaseRequest(table, method, options = {}) {
  ...
  try {
    ...
    if (!response) {
      return { success: false, error: 'RESPONSE_ERROR', message: 'Réponse vide', data: [], count: 0 };
    }
...
try {
  switch (normalizedAction) {
    ...
    case 'create-client': {
      ...
      let type = payload.type || input.type || contextData.type || 'particulier';
      ...
      if (!nom) {
        result = { success: false, error: 'VALIDATION_ERROR', message: 'Le nom est requis' };
        break;
      }
      ...
      if (email) {
        ...
        if (existing.success && existing.count > 0) {
          result = {
            ...
            message: `Client avec email ${email} existe déjà`,
      ...
      if (telephone) {
        ...
        if (existing.success && existing.count > 0) {
          result = {
            success: false,
            error: 'DUPLICATE_CLIENT',
            message: `Client avec téléphone ${telephone} existe déjà`,
            existing_client: existing.data[0]
          };
          break;
        }
        ...
        try {
          ...
          const dossierResult = await supabaseRequest.call(this, 'dossiers', 'POST', {
            body: {
              ...
              description: `Créé auto pour ${nomCompletClient}`,
              statut: 'contact_recu',
              priorite: 'normale',
              source: 'autre'
            }
          ...
          if (dossierResult.success && dossierResult.data && dossierResult.data.length > 0) {
            result.message = `✅ Client créé + Dossier créé`;
            result.dossier = dossierResult.data[0];
            result.dossier_id = dossierResult.data[0].id;
          } else {
            result.message = `✅ Client créé, Dossier non créé`;
          }
        } catch (dossierError) {
          result.message = `✅ Client créé`;
        }
      }
      break;
    }
    ...
    case 'search-client': {
      ...
      if ((!clientsFound.success || clientsFound.count === 0) && q.includes(' ') && searchField === 'nom_complet') {
        ...
        try {
          ...
          if (statusCode >= 200 && statusCode < 300 && Array.isArray(data)) {
            ...
            console.log(`✅ OR: ${data.length} client(s)`);
          }
        } catch (err) {
      ...
      if (result.success) {
        ...
        console.log(`${result.count > 0 ? '✅' : '⚠️'} ${result.count} résultat(s)`);
      }
      break;
    }
    ...
    case 'create-devis': {
        ...
        if (dossierCheck.success && dossierCheck.count > 0) {
          ...
          const dossierResult = await supabaseRequest.call(this, 'dossiers', 'POST', {
            body: {
              client_id: client_id,
              numero: dossierNumero,
              titre: `Dossier ${client.prenom || ''} ${client.nom}`,
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        if (dossierId) result.dossier_id = dossierId;
      }
      break;
    }
        ...
        if (isNumero) {
          ...
          if (clientsResult.success && clientsResult.count > 0) {
            ...
            if (clientIds.length === 1) {
              ...
              console.log(`✅ ${result.count || 0} devis`);
            } else {
              ...
              try {
                ...
                if (statusCode >= 200 && statusCode < 300) {
                  ...
                  console.log(`✅ ${result.count} devis`);
                } else {
                  result = { success: false, error: 'QUERY_ERROR', message: 'Erreur', data: [] };
                }
              } catch (httpError) {
                console.warn('⚠️ Erreur in.() → fallback');
                const allDevis = [];
                ...
                console.log(`✅ ${allDevis.length} devis (fallback)`);
              }
            }
          } else {
            result = { success: true, data: [], count: 0, message: `Aucun client pour "${search}"` };
            console.log(`⚠️ Aucun client`);
          }
        }
      } else {
        console.log(`🔍 Liste tous devis`);
        result = await supabaseRequest.call(this, 'devis', 'GET', {
          select: '*,clients(id,nom,prenom,nom_complet,email,telephone,adresse_facturation,adresse_chantier)',
          limit: payload.limit || 50,
          order: 'date_creation.desc'
    ...
    case 'add-ligne-devis': {
      ...
      if (!lignes || !Array.isArray(lignes) || lignes.length === 0) {
        result = { success: false, error: 'VALIDATION_ERROR', message: 'lignes requis (tableau)' };
        break;
      }
      ...
      if (errors.length > 0) {
        result = {
          success: false,
          error: 'PARTIAL_ERROR',
          message: `${insertedLignes.length} OK, ${errors.length} erreur(s)`,
    ...
    case 'update-ligne-devis': {
      ...
      if (result.success) {
        ...
        if (result.count > 0) result.ligne = result.data[0];
      }
      break;
    }
    ...
    case 'update-devis': {
      const { devis_id, statut, date_envoi, notes } = payload;
    ...
    case 'generate-pdf': {
      ...
      const pdfUrl = `${CONFIG.APP_URL}/api/pdf/devis/${devisUUID}`;
    ...
    case 'envoyer-devis': {
      let { devis_id, email, recipient_email } = payload;
    ...
    case 'creer-facture-depuis-devis': {
      ...
      try {
        ...
        const statusCode = (edgeResponse && edgeResponse.statusCode) || 200;
    ...
    case 'get-facture': {
      const { facture_id, devis_id } = payload;
      
      if (!facture_id && !devis_id) {
        result = { success: false, error: 'VALIDATION_ERROR', message: 'facture_id ou devis_id requis' };
        break;
      }
      
      let factureUUID = facture_id;
      if (facture_id && facture_id.startsWith('FA-')) {
      ...
      if (devis_id && !facture_id) {
        const devisSearchResult = await supabaseRequest.call(this, 'devis', 'GET', {
          search: { numero: devis_id },
          select: 'id',
          limit: 1
        });
        if (devisSearchResult.success && devisSearchResult.count > 0) {
          const facturesResult = await supabaseRequest.call(this, 'factures', 'GET', {
            filters: { devis_id: devisSearchResult.data[0].id },
            select: 'id',
            limit: 1
          });
          if (facturesResult.success && facturesResult.count > 0) {
            factureUUID = facturesResult.data[0].id;
          } else {
            result = { success: false, error: 'NOT_FOUND', message: 'Facture non trouvée' };
            break;
          }
    ...
    case 'list-factures': {
            ...
            if (clientIds.length === 1) {
              ...
              try {
                ...
                if (statusCode >= 200 && statusCode < 300) {
                  ...
                  result = { success: false, error: 'ERROR', data: [], factures: [] };
                }
              } catch (httpError) {
      ...
      if (result.success) {
        result.message = `${result.count} facture(s)${search ? ` pour "${search}"` : ''}`;
      }
      break;
    }
    ...
    case 'list-dossiers': {
      ...
      if (searchTerm && (searchTerm.startsWith('DOS-') || searchTerm.match(/^DOS-\d{4}-\d{3,4}$/))) {
        ...
        // ✅ Recherche client avec 3 stratégies
        let clientsResult = await supabaseRequest.call(this, 'clients', 'GET', {
    ...
    case 'create-rdv': {
      ...
      if (rdvBody.lieu) delete rdvBody.lieu;
    ...
    case 'list-rdv': {
      ...
      if (search) {
        ...
        if (clientsResult.success && clientsResult.count > 0) {
          console.log(`✅ ${clientsResult.count} client(s) → RDV`);
          ...
          if (clientIds.length === 1) {
            ...
            try {
              ...
              if (statusCode >= 200 && statusCode < 300) {
                ...
                result = { success: false, error: 'ERROR', data: [] };
              }
            } catch (httpError) {
              const allRdv = [];
              for (const clientId of clientIds) {
    ...
    default: {
      result = {
        ...
        available: [
          ...
          'get-devis', 'list-devis', 'update-devis', 'delete-devis', 'generate-pdf', 'envoyer-devis',
...
if (!result) {
  result = { success: false, error: 'ERROR', data: [], count: 0 };
}
...
if (result.email_data) finalResult.email_data = result.email_data;
if (result.errors) finalResult.errors = result.errors;
if (result.inserted) finalResult.inserted = result.inserted;
